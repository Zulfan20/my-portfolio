import React, { forwardRef } from 'react'; // <-- PERUBAHAN: Impor forwardRef
import { motion } from 'framer-motion';
import { Eye, Edit, Trash2 } from 'lucide-react';

// --- PERUBAHAN --- Bungkus komponen dengan 'forwardRef' dan tambahkan 'onViewDetail'
const CertificationCard = forwardRef(({ cert, isAdmin, onEdit, onDelete, onViewDetail, ...props }, ref) => {

    return (
        <motion.div 
            ref={ref}   // <-- Terapkan 'ref'
            {...props}  // <-- Terapkan semua prop animasi (layout, initial, animate, exit)
            
            className="bg-white rounded-lg shadow-card overflow-hidden flex flex-col"
            // Hapus animasi 'initial', 'whileInView', 'transition' dari sini
        ...
        >
            <img 
                src={cert.imageUrl || 'https://placehold.co/600x400/e0f2fe/0c4a6e?text=Certificate'} 
                alt={cert.title} 
                className="w-full h-48 object-cover" 
                onError={(e) => e.target.src = 'https://placehold.co/600x400/e0f2fe/0c4a6e?text=Image+Error'}
            />
            {/* --- PERUBAHAN --- Menggunakan flex-col agar tombol rata di bawah */}
            <div className="p-5 flex-1 flex flex-col">
                <h3 className="text-xl font-semibold text-gray-900 mb-1">{cert.title}</h3>
                <p className="text-gray-600 text-sm mb-4 flex-1">{cert.issuer}</p>
                
                {/* --- PERUBAHAN --- Ganti <a> dengan <button> untuk membuka modal --- */}
                <motion.button 
