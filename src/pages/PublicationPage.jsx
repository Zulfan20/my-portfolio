...
import PublicationDetailModal from '../components/PublicationDetailModal.jsx';
...
export default function PublicationPage({ 
    ...
    const [editableContent, setEditableContent] = useState(pubPageContent);

    useEffect(() => {
        setEditableContent(pubPageContent);
    }, [pubPageContent]);

    const onSave = async () => {
        setIsSaving(true);
        try {
            await handleSavePubPageContent(editableContent);
            setIsEditing(false);
        } catch (error) {
            console.error("Error saving pub page content:", error);
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div>
            {isEditing ? (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start mb-6">
                    <div>
                        <h1 className="text-4xl font-bold text-blue-900">Publications</h1>
                        <motion.div 
                ...
                <AnimatePresence>
                    {publications.map((pub) => (
                        <PublicationCard 
                            ...
                            pub={pub} 
                            isAdmin={isAdmin}
                            onEdit={() => setEditingItem({ ...pub, type: 'publications' })}
                            onDelete={() => handleDeleteItem('publications', pub.id)}
                            onViewDetail={() => setSelectedPub(pub)}
